...
Based on [flash-moe](https://github.com/danveloper/flash-moe) by Daniel Woods — extended to work universally.
...
```bash
# Linux / macOS
git clone https://github.com/yourname/flash-moe-universal
...
3. **Pipeline GPU+CPU+IO** — Submit GPU commands without waiting; overlap with next layer's IO
...
| IO | pread + GCD | pread (POSIX) / ReadFile (Windows) |
...
Credits: Daniel Woods (original flash-moe), Claude (coding partner).
